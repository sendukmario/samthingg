...
import TokenImageHoverIgnite from "@/components/customs/TokenImageHoverIgnite";
// ######## Utils & Helpers 🤝 ########
...
export const getRightBadgeType = (dex: DEX, launchpad?: LAUNCHPAD) => {
  // Guard: normalise dex string for safe comparison/transform
  const dexTrimmed = (dex || "")?.trim();
  const launchpadTrimmed = (launchpad || "")?.trim();

  if (dexTrimmed === "LaunchLab" && launchpadTrimmed === "Bonk") {
    return "bonk" as BadgeType;
  }

  if (dexTrimmed === "Dynamic Bonding Curve" && launchpadTrimmed === "Bags") {
    return "bags" as BadgeType;
  }

  if (dexTrimmed === "Dynamic Bonding Curve") {
    if (launchpadTrimmed === "Launch a Coin")
      return "launch_a_coin" as BadgeType;
    if (launchpadTrimmed === "Candle TV") return "candle_tv" as BadgeType;
    if (launchpadTrimmed === "Jupiter Studio")
      return "jupiter_studio" as BadgeType;
  }
...
export const getLeftBadgeType = (
  ...
  const dexTrimmed = (dex || "")?.trim();
  const originTrimmed = (origin_dex || "")?.trim();
  const launchpadTrimmed = (launchpad || "")?.trim();
  ...
  if (graduatedDexList.includes(dexTrimmed)) {
    if (originTrimmed === "LaunchLab" && launchpadTrimmed === "Bonk") {
      return "bonk" as BadgeType;
    }

    if (originTrimmed === "Dynamic Bonding Curve") {
      if (launchpadTrimmed === "Launch a Coin")
        return "launch_a_coin" as BadgeType;
      if (launchpadTrimmed === "Candle TV") return "candle_tv" as BadgeType;
      if (launchpadTrimmed === "Jupiter Studio")
