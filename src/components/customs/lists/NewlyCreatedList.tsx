...
function NewlyCreatedList({
  ...
  const [filterFetchList, setFilterFetchListState] =
    useState<CosmoDataMessageType[]>(newlyCreatedList);
  ...
  useEffect(() => {
    const handleOpenSearch = (
      ...
      const { searchText, triggerSearch } = e.detail;

      setPreviewSearch(searchText);
      ...
      // Trigger the search if requested
      if (triggerSearch) {
        debouncedSetGenuineSearch();
      }
    };
  ...
  }, [debouncedSetGenuineSearch, setPreviewSearch]);
  ...
  const getItemKey = (index: number) => {
    if (isCreatedLoading) return `${index}`;
  ...
  const isCreatedLoading = isLoading || isLoadingFilterFetch;
        ...
        >
          ...
          >
            <div className="absolute left-0 top-0 h-full w-full">
              {filteredList.length > 0 || isCreatedLoading ? (
                <FixedSizeList
                  height={fixedListHeight}
                  width="100%"
                  itemCount={isCreatedLoading ? 30 : filteredList?.length || 0}
                  itemSize={isCreatedLoading ? 95 : cosmoCardHeight}
                  overscanCount={3}
                  itemKey={getItemKey}
                  itemData={itemData}
                >
